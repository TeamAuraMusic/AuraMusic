...
import com.auramusic.app.playback.MusicService.ALBUM
import com.auramusic.app.playback.MusicService.ARTIST
import com.auramusic.app.playback.MusicService.PLAYLIST
import com.auramusic.app.playback.MusicService.ROOT
import com.auramusic.app.playback.MusicService.SHUFFLE_ACTION
import com.auramusic.app.playback.MusicService.SONG
import com.auramusic.app.playback.MusicService.YOUTUBE_PLAYLIST
import com.auramusic.app.playback.MusicService.SEARCH
        ...
        scope.future(Dispatchers.IO) {
            LibraryResult.ofItemList(
                when (parentId) {
                    ...
                    SONG -> database.songsByCreateDateAsc().first()
                    ...
                    PLAYLIST -> {
                        ...
                        listOf(
                            browsableMediaItem(
                                "${PLAYLIST}/${PlaylistEntity.LIKED_PLAYLIST_ID}",
                            ...
                            browsableMediaItem(
                                "${PLAYLIST}/${PlaylistEntity.DOWNLOADED_PLAYLIST_ID}",
                        ...
                        youtubePlaylists.map { ytPlaylist ->
                            browsableMediaItem(
                                "${YOUTUBE_PLAYLIST}/${ytPlaylist.id}",
                                ytPlaylist.title,
                        ...
                        when {
                            parentId.startsWith("${ARTIST}/") ->
                                database.artistSongsByCreateDateAsc(parentId.removePrefix("${ARTIST}/"))
                                    .first().map {
                                    it.toMediaItem(parentId)
                                }

                            parentId.startsWith("${ALBUM}/") ->
                                database.albumSongs(parentId.removePrefix("${ALBUM}/"))
                                    .first().map {
                                    it.toMediaItem(parentId)
                                }
                            ...
                            parentId.startsWith("${PLAYLIST}/") -> {
                                ...
                                listOf(
                                    MediaItem.Builder()
                                        .setMediaId("$parentId/${SHUFFLE_ACTION}")
                                        .setMediaMetadata(
                            ...
                            parentId.startsWith("${YOUTUBE_PLAYLIST}/") -> {
                                val playlistId = parentId.removePrefix("${YOUTUBE_PLAYLIST}/")
                                try {
            ...
            try {
                ...
                try {
                    ...
                    onlineResults.forEach { songItem ->
                        ...
                        searchResults.add(
                            MediaItem.Builder()
                                .setMediaId("${SEARCH}/$query/${songItem.id}")
                                .setMediaMetadata(
                                    MediaMetadata.Builder()
                                        .setTitle(songItem.title)
                                        .setSubtitle(songItem.artists.joinToString(", ") { it.name })
